...
class ImportViewSet(ProjectLookupMixin, ViewSet):
    ...
    def prescan(self, request, project_slug=None, pk=None):
        ...
        if not prescan_data:
            return Response(
                {'detail': 'Prescan not yet complete'},
    ...
    def start(self, request, project_slug=None, pk=None):
        ...
        # Update job config
        job.job_config['selected_load_cases'] = serializer.validated_data.get('selected_load_cases', [])
        job.job_config['conflict_resolution'] = conflict_dict
        job.job_config['result_set_name'] = serializer.validated_data.get('result_set_name', 'Imported Results')
