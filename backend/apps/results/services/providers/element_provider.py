...
def get_element_results(
    ...
    cache_entries = ElementResultsCache.objects.filter(
        project=service.project,
        result_set_id=result_set_id,
        result_type=cache_type,
        element_id=element_id,
    ).select_related('story', 'element').order_by('-story_sort_order')
...
def get_all_elements_for_type(
    ...
    ).select_related('element').values(
        'element__id', 'element__name', 'element__element_type'
    ).distinct()
